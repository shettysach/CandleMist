...
pub fn TypeArea(send: Action<String, Result<(), ServerFnError>>) -> impl IntoView {
    let type_area_class = format!("{INPUT_AREA_CLASS} {INPUT_AREA_CLASS_DARK}");
    let text_area_class = format!("{TEXT_AREA_CLASS} {TEXT_AREA_CLASS_DARK}");
    let button_class = format!("{BUTTON_CLASS} {BUTTON_CLASS_DARK}");
